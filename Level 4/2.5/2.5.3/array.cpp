...
Array::Array(unsigned int size) : m_size(size), m_data(new Point[size]) {}
...
// SetElement
void Array::SetElement(const Point& p, unsigned int idx)
{
...
const Point& Array::GetElement(unsigned int idx) const
{
	// handle index out of bounds
	if (idx >= m_size)
		return m_data[0]; 
...
Point& Array::GetElement(unsigned int idx)
{
	// handle index out of bounds
	if (idx >= m_size)
		return m_data[0];
...
// [] operator
Point& Array::operator [] (unsigned int idx)
{
...
// const [] operator
// read only access returned, safe access
const Point& Array::operator [] (unsigned int idx) const
{
	// handle index out of bonds
	if (idx >= m_size)
		return m_data[0];
